...
namespace JaxQx
{
    internal class AssassinManager
    {
        ...
        private static void Load()
        {
            ...
            LocalMenu = new Menu("JaxQx | Target Selector", "TSxQx");

            Program.Config.AddSubMenu(LocalMenu);
            LocalMenu.AddItem(new MenuItem("TS.Mode", "Selector Mode:").SetValue(new StringList(new[] { "L# Target Selector", "Jax Target Selector" }, 1)));
            LocalMenu.AddItem(new MenuItem("TS.Active", "Active").SetValue(true));
            LocalMenu.AddItem(new MenuItem("TS.SearchRange", MenuTab + "Enemy Searching Range")).SetValue(new Slider(1400, 2000));
