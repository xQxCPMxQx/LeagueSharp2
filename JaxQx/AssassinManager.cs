...
namespace JaxQx
{
    internal class AssassinManager
    {
        ...
        private static float SelectorRange
        {
            get
            {
                return Program.Config.Item("Enemies.SearchRange").GetValue<Slider>().Value;
            }
        }
        ...
        private static Obj_AI_Hero TsEnemy
        {
            get
            {
                ...
                var vMax =
                    HeroManager.Enemies.Where(e => !e.IsDead && e.IsVisible && e.IsValidTarget(SelectorRange) && !e.IsZombie)
                        .Max(h => LocalMenu.Item("Selected" + h.ChampionName).GetValue<StringList>().SelectedIndex);
        ...
        private static void Load()
        {
            ...
            LocalMenu.AddItem(new MenuItem("Enemies.Enemies.Title", "Enemies:").SetFontStyle(FontStyle.Regular, SharpDX.Color.GreenYellow));
            {
                foreach (var enemy in HeroManager.Enemies)
                {
                    LocalMenu.AddItem(
                        new MenuItem("Selected" + enemy.ChampionName, MenuTab + enemy.CharData.BaseSkinName).SetValue(
                            new StringList(
                                new[] { "Low Focus", "Medium Focus", "High Focus" },
                                GetPriority(enemy.ChampionName))));
                }
            }

            LocalMenu.AddItem(new MenuItem("Enemies.Other.Title", "Other Settings:").SetFontStyle(FontStyle.Regular, SharpDX.Color.GreenYellow));
            {
                LocalMenu.AddItem(
                    new MenuItem("Enemies.AutoPriority Focus", MenuTab + "Auto arrange priorities").SetShared()
                        .SetValue(false)).ValueChanged += AutoPriorityItemValueChanged;
            }
            LocalMenu.AddItem(
                new MenuItem("Enemies.Click", MenuTab + "Chance Enemy's Hitchance with Mouse Left-click").SetValue(true));

            LocalMenu.AddItem(new MenuItem("Draw.Title", "Drawings").SetFontStyle(FontStyle.Regular, SharpDX.Color.GreenYellow));
            {
                LocalMenu.AddItem(
                    new MenuItem("Draw.Search", MenuTab + "Show Search Range").SetValue(
        ...
        private static void Game_OnWndProc(WndEventArgs args)
        {
            ...
            if (!Program.Config.Item("Enemies.Click").GetValue<bool>()) return;
            ...
            var selectedTarget =
                HeroManager.Enemies.FindAll(hero => hero.IsValidTarget() && hero.Distance(Game.CursorPos, true) < 40000)
                    ...
                    .FirstOrDefault();
            {
                if (selectedTarget == null || !selectedTarget.IsVisible) return;

                var vSelected =
                    LocalMenu.Item("Selected" + selectedTarget.ChampionName).GetValue<StringList>().SelectedIndex;
            ...
            string[] highPriority =
                {
                    ...
                    "Katarina", "Kennen", "KogMaw", "Leblanc", "Lucian", "Lux", "Malzahar", "MasterYi",
                    "MissFortune", "Orianna", "Quinn", "Sivir", "Syndra", "Talon", "Teemo", "Tristana",
                    "TwistedFate", "Twitch", "Varus", "Vayne", "Veigar", "VelKoz", "Viktor", "Xerath",
                    "Zed", "Ziggs"
                };
        ...
        {
            if (Program.Config.Item("Enemies.Mode").GetValue<StringList>().SelectedIndex == 0) return;
            ...
            if (Program.Config.Item("Draw.Status").GetValue<bool>())
            {
                foreach (var enemy in HeroManager.Enemies.Where(enemy => enemy.IsVisible && !enemy.IsDead))
                {
                    ...
                    if (LocalMenu.Item("Draw.Status.Show").GetValue<StringList>().SelectedIndex == 1 && vSelected != 2) continue;

                    Utils.DrawText(
                        vSelected == 2 ? Utils.TextBold : Utils.Text,
            ...
            {
                Render.Circle.DrawCircle(ObjectManager.Player.Position, SelectorRange, drawSearch.Color, 1);
            }
        }
