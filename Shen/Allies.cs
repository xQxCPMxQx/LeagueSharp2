    ...
    internal class Allies
    {
        ...
        public static Obj_AI_Hero GetAlly
        {
            get
            {
                var vMax = HeroManager.Allies.Where(
                    a =>
                        !a.IsDead && !a.IsMe && a.CountEnemiesInRange(UnderAllyTurret(a) ? 350 : 350 + 350) > 0 &&
                        a.Health < 400)
                    .Max(
        ...
        private static void Load()
        {
            ...
            LocalMenu.AddItem(new MenuItem("ChampionAllies.ChampionAllies.Title", "Protection Status:"));
            {
                ...
                {
                    LocalMenu.AddItem(
                        new MenuItem("Selected.Champ" + ally.ChampionName, MenuTab + ally.ChampionName).SetValue(
                            new StringList(new[] {"Off", "Low Protection", "Medium Protection", "High Protection"},
                                GetProtection(ally.ChampionName))));
                }
            }
            LocalMenu.AddItem(
                new MenuItem("Allies.Ulti", "Use Ulti for :").SetValue(
            ...
            {
                LocalMenu.AddItem(
                    new MenuItem("ChampionAllies.AutoProtection", MenuTab + "Auto Arrange Protection").SetShared()
