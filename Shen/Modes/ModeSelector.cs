...
namespace Shen.Modes
{
    ...
    internal class ModeSelector
    {
        ...
        public static void Initialize(LeagueSharp.Common.Menu mainMenu)
        {
            ...
            LocalMenu.AddItem(
                new MenuItem("TS", "Active Target Selector:").SetValue(
                    new StringList(new[] { "Shen Target Selector", "L# Target Selector" })))
            ...
            LocalMenu.AddItem(new MenuItem("ModeSelector.Range", "Range (Recommend: Max):"))
                .SetValue(new Slider((int)(  Shen.Champion.PlayerSpells.E.Range * 3), 
                                        (int)Shen.Champion.PlayerSpells.E.Range, 
                                        (int)(Shen.Champion.PlayerSpells.Q.Range * 5))).SetTag(11);
            ...
            {
                LocalMenu.AddItem(
                    new MenuItem("enemy_" + e.ChampionName, $"{Shen.Common.DrawHelper.MenuTab}Focus {e.ChampionName}")
            ...
            LocalMenu.AddItem(new MenuItem("Draw.Range", Shen.Common.DrawHelper.MenuTab + "ModeSelector.Range").SetValue(new Circle(true,System.Drawing.Color.Gray)).SetTag(11));
            LocalMenu.AddItem(new MenuItem("Draw.Enemy", Shen.Common.DrawHelper.MenuTab + "ActiveJungle Enemy").SetValue(new Circle(true,System.Drawing.Color.GreenYellow)).SetTag(11));
            LocalMenu.AddItem(new MenuItem("Draw.Status", Shen.Common.DrawHelper.MenuTab + "Show Enemy:").SetValue(new StringList(new[] { "Off", "Text", "Picture", "Line", "All" }, 0)));
                    ...
                    if (objAiHero != null && objAiHero.IsVisible && !objAiHero.IsDead)
                    {
                        ...
                        switch (set)
                        {
                            case 0:
                                {
                                    ClearAssassinList();
                                    MenuConfig.LocalMenu.Item("enemy_" + objAiHero.ChampionName).SetValue(true);
                                    break;
                                }
                            case 1:
                                {
                                    var menuStatus =
                                        MenuConfig.LocalMenu.Item("enemy_" + objAiHero.ChampionName).GetValue<bool>();
                                    MenuConfig.LocalMenu.Item("enemy_" + objAiHero.ChampionName).SetValue(!menuStatus);
                                    break;
                                }
                        }
                    }
        ...
        private static void Drawing_OnDraw(EventArgs args)
        {
            ...
            if (drawEnemy.Active)
            {
                var t = GetTarget(Shen.Champion.PlayerSpells.Q.Range, LeagueSharp.Common.TargetSelector.DamageType.Physical);
                if (t.IsValidTarget())
                {
        ...
        private static Obj_AI_Hero KillableEnemy
        {
            ...
            get
            {
                Obj_AI_Hero t = GetTarget(Shen.Champion.PlayerSpells.Q.Range);
