...
//                    Game.PrintChat("W Posision : " + existingSlide.Position);
 //                   Game.PrintChat("Target Position : " + vTarget.Position);
        ...
        private static void Game_OnGameUpdate(EventArgs args)
        {
            ...
            {
                var huntRange = TargetSelectorMenu.Item("HuntRange").GetValue<Slider>().Value;
                Obj_AI_Hero vTarget = null;
                foreach (var enemy in ObjectManager.Get<Obj_AI_Hero>()
                    .Where(enemy => enemy.Team != Player.Team
                        && !enemy.IsDead && enemy.IsVisible
                        && TargetSelectorMenu.Item("Hunt" + enemy.BaseSkinName) != null
                        && TargetSelectorMenu.Item("Hunt" + enemy.BaseSkinName).GetValue<bool>())
                        .OrderBy(enemy => enemy.Distance(Game.CursorPos))
                        )
                {
                    
                    vTarget = Player.Distance(enemy) < huntRange ? enemy : null;
                }
                Combo(vTarget);
            }
            else
