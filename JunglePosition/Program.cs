...
namespace JunglePosition
{
    class Program
    {
        ...
        private static void Game_OnGameLoad(EventArgs args)
        {
            ...
            Config.AddItem(new MenuItem("MapT", "Map").SetValue(new StringList(new[] { "Old Map", "New Map"}, 1)));
        ...
        {
            var xMap = Config.Item("MapT").GetValue<StringList>();
            var show = Config.Item("Show").GetValue<Circle>();
            if (show.Active)
            {
                var circleRange = 75f;

                switch (xMap.SelectedIndex)
                {
                    case 0:
                        {
