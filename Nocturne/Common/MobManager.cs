...
namespace Nocturne.Common
{
    internal class MobManager
    {
        ...
        public static void Initialize(Menu mainMenu)
        {
            ...
            {
                string[] timeRemaining = new[] {"Off", "10 secs. remain to respawn", "20 secs. remain to respawn", "30 secs. remain to respawn" };
                ...
                LocalMenu.AddItem(new MenuItem("BuffManager.JungleTimerFormat", "Display Format").SetValue(new StringList(new[] { "m:ss", "ss" })));

                LocalMenu.AddItem(new MenuItem("JungleActive", "Jungle Timer").SetValue(true));

                JungleMobs.Add(new JungleCamp("SRU_Blue", 300, new Vector3(3871.489f, 7901.054f, 51.90324f), new[] { "SRU_Blue1.1.1", "SRU_BlueMini1.1.2", "SRU_BlueMini21.1.3" }));

                JungleMobs.Add(new JungleCamp("SRU_Red", 300, new Vector3(7862f, 4112f, 53.71951f), new[] { "SRU_Red4.1.1", "SRU_RedMini4.1.2", "SRU_RedMini4.1.3" }));

                JungleMobs.Add(new JungleCamp("SRU_Dragon", 360, new Vector3(9866.148f, 4414.014f, -71.2406f), new[] { "SRU_Dragon6.1.1" }));

                JungleMobs.Add(new JungleCamp("SRU_Blue", 300, new Vector3(10931.73f, 6990.844f, 51.72291f), new[] { "SRU_Blue7.1.1", "SRU_BlueMini7.1.2", "SRU_BlueMini27.1.3" }));

                JungleMobs.Add(new JungleCamp("SRU_Red", 300, new Vector3(7016.869f, 10775.55f, 56.00922f), new[] { "SRU_Red10.1.1", "SRU_RedMini10.1.2", "SRU_RedMini10.1.3" }));

                JungleMobs.Add(new JungleCamp("SRU_Baron", 420, new Vector3(5007.124f, 10471.45f, -71.2406f), new[] { "SRU_Baron12.1.1" }));
        ...
        private static void Game_OnGameUpdate(EventArgs args)
        {
            ...
            if ((int)Game.ClockTime - _nextTime >= 0)
            {
                ...
                IEnumerable<JungleCamp> junglesAlive = JungleMobs.Where(jungle => !jungle.IsDead && jungle.Names.Any(s => minions.Where(minion => minion.Name == s).Select(minion => minion.Name).FirstOrDefault() != null));
                foreach (JungleCamp jungle in junglesAlive)
                {
                    jungle.Visibled = true;
                }

                IEnumerable<JungleCamp> junglesDead = JungleMobs.Where(jungle => !jungle.IsDead && jungle.Visibled && jungle.Names.All(s => minions.Where(minion => minion.Name == s).Select(minion => minion.Name).FirstOrDefault() == null));
