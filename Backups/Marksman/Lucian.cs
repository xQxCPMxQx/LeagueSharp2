...
namespace Marksman
{
    internal class Lucian : Champion
    {
        ...
        private float GetComboDamage(Obj_AI_Hero t)
        {
            ...
            float xAttackDmg = ObjectManager.Player.BaseAttackDamage + ObjectManager.Player.FlatPhysicalDamageMod;

            if (Q.IsReady())
                fComboDamage += (float) ObjectManager.Player.GetSpellDamage(t, SpellSlot.Q) + xAttackDmg * 2;

            if (W.IsReady())
                fComboDamage += (float) ObjectManager.Player.GetSpellDamage(t, SpellSlot.W) + xAttackDmg * 2;
