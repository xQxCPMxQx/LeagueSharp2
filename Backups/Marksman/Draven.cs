...
namespace Marksman
{
    ...
    internal class Draven : Champion
    {
        ...
        {
            if (Math.Abs(ObjectManager.Player.ServerPosition.Distance(position)) < 0.00001)
            {
                ObjectManager.Player.IssueOrder(GameObjectOrder.HoldPosition, ObjectManager.Player.ServerPosition);
                return;
            }

            if (ObjectManager.Player.ServerPosition.Distance(position) < 100)
            {
                if (ObjectManager.Player.Path.Count() > 1)
                    ObjectManager.Player.IssueOrder(GameObjectOrder.HoldPosition, ObjectManager.Player.ServerPosition);
                return;
            }

            if (position.Distance(ObjectManager.Player.Position) < 200)
                ObjectManager.Player.IssueOrder(GameObjectOrder.MoveTo, position);
            else
            {
                var point = ObjectManager.Player.ServerPosition +
                            200 * (position.To2D() - ObjectManager.Player.ServerPosition.To2D()).Normalized().To3D();
                ...
                ExistingReticles.OrderBy(reticle => reticle.ExpireTime)
                    .FirstOrDefault(
                        reticle =>
                            ...
                            ObjectManager.Player.GetPath(reticle.ReticlePos).ToList().To2D().PathLength() /
                            ObjectManager.Player.MoveSpeed + Game.Time < reticle.ExpireTime);

            return reticles != null && ObjectManager.Player.Distance(reticles.ReticlePos) >= 100
                ? reticles.ReticlePos
                : cursor;
        }
