...
namespace Irelia
{
    internal class Program
    {
        ...
        private static void Game_OnGameLoad(EventArgs args)
        {
            ...
            //Interrupter.OnPosibleToInterrupt += OnProcessSpellCast;
            QUsedTime = Game.Time;
...
//            if ((vIrelia.Spellbook.GetSpell(SpellSlot.W).Level) > 0)
//                xComboDamage += W.GetDamage(vTarget);
        ...
        private static bool İsStunPossible(Obj_AI_Base vTarget)
        {
        ...
        private static void OnProcessSpellCast(Obj_AI_Base vTarget, GameObjectProcessSpellCastEventArgs args)
        {
            ...
            if (stopUlties)
            {
                ...
                foreach (string interruptSpellName in interruptSpells)
                {
                    ...
                    {
                        if (vIrelia.Distance(vTarget) <= E.Range && E.IsReady() && İsStunPossible(vTarget)) /* stun possible */
                            E.CastOnUnit(vTarget);
                    }
                }
            }
